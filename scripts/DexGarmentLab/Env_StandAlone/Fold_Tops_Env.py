...
class FoldTops_Env(BaseEnv):
    def __init__(
        ...
        self.garment = Particle_Garment(
            ...
            usd_path=os.path.join("/workspace/isaaclab/scripts/DexGarmentLab/Assets/Garment/Tops/Collar_Lsleeve_FrontClose/TCLC_018/TCLC_018_obj.usd") if usd_path is None else usd_path,
        ...
        #     use_topology=True,
        #     topology_checkpoint="Model_HALO/GAM/checkpoints/Tops_LongSleeve/topology_finetune_5000.pth",
...
def FoldTops(pos, ori, usd_path, ground_material_usd, data_collection_flag, record_video_flag):
    ...
    pcd, color = env.garment_camera.get_point_cloud_data_from_segment(
        ...
        # save_path=get_unique_filename(f"{usd_path}", extension=".ply"),
    ...
    if record_video_flag and success:
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/video"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/video")
        env.env_camera.create_mp4(get_unique_filename("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/video/video", ".mp4"))

    if data_collection_flag:
        # write into .log file
        with open("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/data_collection_log.txt", "a") as f:
            f.write(f"result:{success}  usd_path:{env.garment.usd_path}  pos_x:{pos[0]}  pos_y:{pos[1]}\n")
        if success:
            env.record_to_npz()
            if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/final_state_pic"):
                os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/final_state_pic")
            env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops/final_state_pic/img",".png"))
...
if __name__=="__main__":
    ...
    if args.garment_random_flag:
        ...
        # assets_lists = os.path.join("/workspace/isaaclab/scripts/DexGarmentLab/Model_HALO/GAM/checkpoints/Tops_LongSleeve/assets_training_list.txt")
        assets_lists = os.path.join("/workspace/isaaclab/scripts/DexGarmentLab/Model_HALO/GAM/checkpoints/Trousers/assets_training_list.txt")
        # assets_lists = os.path.join("/workspace/isaaclab/scripts/DexGarmentLab/Model_HALO/GAM/checkpoints/Dress_LongSleeve/assets_training_list.txt")
        assets_list = []
