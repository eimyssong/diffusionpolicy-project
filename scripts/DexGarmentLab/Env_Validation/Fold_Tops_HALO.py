...
class FoldTops_Env(BaseEnv):
    def __init__(
        ...
        self.garment = Particle_Garment(
            ...
            usd_path="/workspace/isaaclab/scripts/DexGarmentLab/Assets/Garment/Tops/Collar_Lsleeve_FrontClose/TCLC_018/TCLC_018_obj.usd" if usd_path is None else usd_path,
        ...
        ######################################################################################################
        # # from Model_HALO.GAM.model.train_topology_finetune import train

        # # train()

        # # self.model = GAM_Encapsulation(
        # #     catogory="Tops_LongSleeve",
        # #     use_topology=True,
        # #     topology_checkpoint="Model_HALO/GAM/checkpoints/Tops_LongSleeve/topology_finetune.pth",
        # #     input_garment_type="trousers",
        # # )
...
def FoldTops(pos, ori, usd_path, ground_material_usd, validation_flag, record_video_flag, training_data_num, stage_1_checkpoint_num, stage_2_checkpoint_num, stage_3_checkpoint_num):
    ...
    save_dir = "/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/correspondence_vis"
    ...
    # manipulation_points, indices, points_similarity = env.model.get_manipulation_points(input_pcd=pcd, index_list=[1902, 501, 957, 448, 1196, 422])
    # manipulation_points, indices, points_similarity = env.model.get_manipulation_points(input_pcd=pcd, index_list=[957, 1902, 448, 501, 1196, 422])
    # manipulation_points, indices, points_similarity = env.model.get_manipulation_points(input_pcd=pcd, index_list=[957, 501, 1902, 448, 501, 448])
    ...
    if record_video_flag:
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/video"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/video")
        env.env_camera.create_mp4(get_unique_filename("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/video/video", ".mp4"))
    ...
    if validation_flag:
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO")
        # write into .log file
        with open("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/validation_log.txt", "a") as f:
            f.write(f"result:{success}  usd_path:{env.garment.usd_path}  pos_x:{pos[0]}  pos_y:{pos[1]}\n")
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/final_state_pic"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Tops_Validation_HALO/final_state_pic/img",".png"))
