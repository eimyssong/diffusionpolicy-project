...
class FoldTrousers_Env(BaseEnv):
    def __init__(
        ...
        self.garment = Particle_Garment(
            ...
            usd_path="/workspace/isaaclab/scripts/DexGarmentLab/Assets/Garment/Trousers/Long/PL_Pants036/PL_Pants036_obj.usd" if usd_path is None else usd_path,
        )
        ...
        ################################################################################################################
        self.model = GAM_Encapsulation(catogory="Trousers", force_finetune=True)        
        
        ################################################################################################################

        
        self.sadp_g_stage_1 = SADP_G(task_name="Fold_Trousers_stage_1", data_num=training_data_num, checkpoint_num=stage_1_checkpoint_num)

        self.sadp_g_stage_2 = SADP_G(task_name="Fold_Trousers_stage_2", data_num=training_data_num, checkpoint_num=stage_2_checkpoint_num)    
        

        self.sadp_g_stage_3 = SADP_G(
            task_name="Fold_Trousers_stage_3_source_target",
        ...
        world_bbox = bbox_cache.ComputeWorldBound(mesh_prim)
        world_range = world_bbox.ComputeAlignedRange()

        bbox_min = np.array(world_range.GetMin())
        bbox_max = np.array(world_range.GetMax())
        garment_size = bbox_max - bbox_min

        print("\n========== Garment Size ==========")
        print("mesh path:", mesh_path)
        print("metersPerUnit:", UsdGeom.GetStageMetersPerUnit(self.stage))
        print("bbox min:", bbox_min)
        print("bbox max:", bbox_max)
        print("size X:", garment_size[0], "m")
        print("size Y:", garment_size[1], "m")
        print("size Z:", garment_size[2], "m")
        print("==================================\n")
...
def FoldTrousers(pos, ori, usd_path, ground_material_usd, validation_flag, record_video_flag, training_data_num, stage_1_checkpoint_num, stage_2_checkpoint_num, stage_3_checkpoint_num):
    ...
    # save_dir = "/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/correspondence_vis"
    ...
    env.model.visualize_pcd_corresponce(
        input_pcd=pcd_rotate,
        save_or_not=False, # True, False #############################################################################################################
        save_path=get_unique_filename(
            f"{save_dir}/vis",
            ".ply"
        )
    ...
    manipulation_points, indices, points_similarity = env.model.get_manipulation_points(input_pcd=pcd_rotate, index_list=[983, 521, 1801, 471, 1170, 214, 338, 1222])
    ...
    # #new stage    #####################################################################################################################
    # # env.points_affordance_feature = normalize_columns(np.concatenate([points_similarity[6:7], points_similarity[6:7]], axis=0).T)
    # env.points_affordance_feature = normalize_columns(points_similarity[[6, 7]].T)

    # pick_ori = np.array([0.406, -0.406, -0.579, 0.579])
    # # pick_ori = np.array([0.0, 0.0, 0.707, -0.707])

    # success = env.bimanual_dex.dexright.dense_step_action(
    #     target_pos=np.array([manipulation_points[6][0], manipulation_points[6][1], 0.08]),
    #     target_ori=pick_ori,
    #     angular_type="quat",
    #     dense_sample_scale=0.005,
    # )

    # current_pos, current_ori = env.bimanual_dex.dexright.get_cur_ee_pos()

    # print("target:", [manipulation_points[6][0], manipulation_points[6][1], 0.06])
    # print("actual EE:", current_pos)
    # print("success:", success)


    # # # catch garment
    # # env.bimanual_dex.dexright.dense_step_action(
    # #     target_pos=np.array([0.6, manipulation_points[6][1]+0.02, 0.2]),
    ...
    # # # catch garment
    # # env.bimanual_dex.dexright.dense_step_action(
    # #     target_pos=np.array([manipulation_points[6][0], manipulation_points[6][1], 0.20]),
    # #     target_ori=np.array([0.0, 0.0, 0.707, -0.707]),   
    # #     angular_type="quat",                                                                                 
    # # )
    ...
    # # # catch garment
    # # env.bimanual_dex.dexright.dense_step_action(
    # #     target_pos=np.array([manipulation_points[6][0], manipulation_points[6][1], 0.05]),
    # #     # target_ori=np.array([0.0, 0.0, 0.707, -0.707]),   
    # #     target_ori=pick_ori, 
    # #     angular_type="quat",                                                                                 
    # # )
    ...
    # # pick_ori = np.array([0.406, -0.406, -0.579, 0.579])
    # pick_ori = np.array([0.579, -0.579, -0.406, 0.406])
    # # pick_ori = np.array([0.0, 0.0, 0.707, -0.707])

    # success = env.bimanual_dex.dexleft.dense_step_action(
    #     target_pos=np.array([manipulation_points[6][0], manipulation_points[6][1], 0.0]),
    #     target_ori=pick_ori,
    #     angular_type="quat",
    #     dense_sample_scale=0.005,
    # )

    # current_pos, current_ori = env.bimanual_dex.dexleft.get_cur_ee_pos()

    # print("target:", [manipulation_points[6][0], manipulation_points[6][1], 0.20])
    # print("actual EE:", current_pos)
    # print("success:", success)
    ...
    if record_video_flag:
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/video"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/video")
        env.env_camera.create_mp4(get_unique_filename("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/video/video", ".mp4"))
    ...
    if validation_flag:
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO")
        # write into .log file
        with open("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/validation_log.txt", "a") as f:
            f.write(f"result:{success}  usd_path:{env.garment.usd_path}  pos_x:{pos[0]}  pos_y:{pos[1]}\n")
        if not os.path.exists("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/final_state_pic"):
            os.makedirs("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("/workspace/isaaclab/scripts/DexGarmentLab/Data/Fold_Trousers_Validation_HALO/final_state_pic/img",".png"))
...
if __name__=="__main__":
    ...
    if args.garment_random_flag:
        ...
        # x = np.random.uniform(-0.05, 0.15) # changeable
        y = np.random.uniform(0.7, 0.8) # changeable
        # y = np.random.uniform(0.6, 0.7) # changeable
    ...
    # usd_path = "Assets/Garment/Dress/Long_LongSleeve/DLLS_Dress008_0/DLLS_Dress008_0_obj.usd"
    # usd_path = '/workspace/isaaclab/scripts/DexGarmentLab/Assets/Garment/Dress/Short_LongSleeve/DSLS_Dress149/DSLS_Dress149_obj.usd'
    # usd_path = '/workspace/isaaclab/scripts/DexGarmentLab/Assets/Garment/Dress/Short_LongSleeve/DSLS_Dress405/DSLS_Dress405_obj.usd'
    # usd_path='Assets/Garment/Dress/Long_LongSleeve/DLLS_Dress008_1/DLLS_Dress008_1_obj.usd'
    # usd_path='Assets/Garment/Dress/Short_LongSleeve/DSLS_Dress233/DSLS_Dress233_obj.usd'
    # usd_path='Assets/Garment/Dress/Short_LongSleeve/DSLS_Dress405/DSLS_Dress405_obj.usd'
    # usd_path='Assets/Garment/Dress/Short_LongSleeve/DLLS_Dress054_1/DLLS_Dress054_1_obj.usd'
    # usd_path='Assets/Garment/Dress/Short_LongSleeve/DSLS_Dress214/DSLS_Dress214_obj.usd'
    # usd_path='Assets/Garment/Tops/NoCollar_noSleeve_FrontClose/TNNC_model2_021/TNNC_model2_021_obj.usd'
    # usd_path='Assets/Garment/Trousers/Long/PL_M1_080/PL_M1_080_obj.usd'
