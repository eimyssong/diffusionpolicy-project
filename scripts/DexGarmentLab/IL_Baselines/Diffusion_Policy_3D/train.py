...
sys.path.append("IL_Baselines/Diffusion_Policy_3D")
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000, task_name="Hang_Tops_stage_1"):
        ...
        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(
                f"IL_Baselines/Diffusion_Policy_3D/checkpoints/{task_name}/{checkpoint_num}.ckpt"
            )
        else:
            ckpt_file = pathlib.Path(
                f"IL_Baselines/Diffusion_Policy_3D/checkpoints/{task_name}_w_rgb/{checkpoint_num}.ckpt"
            )
