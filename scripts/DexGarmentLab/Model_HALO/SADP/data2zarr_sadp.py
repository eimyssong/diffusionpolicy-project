...
def main():
    ...
    for current_ep in tqdm(range(train_data_num), desc=f"Processing {train_data_num} MetaData"):
        ...
        for i in range(data_length-1):
            ...
            points_affordance_feature_arrays.append(meta_data[i]['points_affordance_feature'])
            # state and action
        ...
        if (current_ep + 1) % batch_size == 0 or (current_ep + 1) == train_data_num:
            ...
            points_affordance_feature_arrays = np.array(points_affordance_feature_arrays)
            action_arrays = np.array(action_arrays)
            state_arrays = np.array(state_arrays)
            episode_ends_arrays = np.array(episode_ends_arrays)

            print("environment_point_cloud_arrays shape : ", environment_point_cloud_arrays.shape)
            print("garment_point_cloud_arrays shape : ", garment_point_cloud_arrays.shape)
            print("object_point_cloud_arrays shape : ", object_point_cloud_arrays.shape)
            print("points_affordance_feature_arrays shape : ", points_affordance_feature_arrays.shape)
            ...
            zarr_data["points_affordance_feature"].append(points_affordance_feature_arrays)
