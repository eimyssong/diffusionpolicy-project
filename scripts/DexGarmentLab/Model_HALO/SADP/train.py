...
sys.path.append('Model_HALO/SADP')

from structure_aware_diffusion_policy.common.checkpoint_util import TopKCheckpointManager
from structure_aware_diffusion_policy.common.pytorch_util import dict_apply, optimizer_to
from structure_aware_diffusion_policy.dataset.base_dataset import BaseDataset
from structure_aware_diffusion_policy.env_runner.base_runner import BaseRunner
from structure_aware_diffusion_policy.model.common.lr_scheduler import get_scheduler
from structure_aware_diffusion_policy.model.diffusion.ema_model import EMAModel
from structure_aware_diffusion_policy.policy.sadp import SADP
from hydra.core.hydra_config import HydraConfig

OmegaConf.register_new_resolver("eval", eval, replace=True)

class TrainSADPWorkspace:
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        print("SADP:", os.getcwd())
        # configure model
        self.model: SADP = hydra.utils.instantiate(cfg.policy)

        self.ema_model: SADP = None
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000, task_name="Hang_Tops_stage_1"):
        ...
        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(
                f"Model_HALO/SADP/checkpoints/{task_name}/{checkpoint_num}.ckpt"
            )
        else:
            ckpt_file = pathlib.Path(
                f"Model_HALO/SADP/checkpoints/{task_name}_w_rgb/{checkpoint_num}.ckpt"
            )
...
@hydra.main(
    version_base=None,
    config_path=str(
        pathlib.Path(__file__).parent.joinpath("structure_aware_diffusion_policy", "config")
    ),
)
def main(cfg):
    workspace = TrainSADPWorkspace(cfg)
    workspace.run()
