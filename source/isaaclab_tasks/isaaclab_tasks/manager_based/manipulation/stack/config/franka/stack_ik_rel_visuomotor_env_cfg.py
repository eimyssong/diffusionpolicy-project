...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        table_cam_depth = ObsTerm(
            func=mdp.image, params={"sensor_cfg": SceneEntityCfg("table_cam"), "data_type": "distance_to_image_plane", "normalize": False}
        )
        wrist_cam_depth = ObsTerm(
            func=mdp.image, params={"sensor_cfg": SceneEntityCfg("wrist_cam"), "data_type": "distance_to_image_plane", "normalize": False}
        )
        table_cam_pointcloud = ObsTerm(
            func=mdp.point_cloud, params={"sensor_cfg": SceneEntityCfg("table_cam")}
        )
...
class FrankaCubeStackVisuomotorEnvCfg(stack_joint_pos_env_cfg.FrankaCubeStackEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.table_cam = CameraCfg(
            ...
            spawn=sim_utils.PinholeCameraCfg(
                # focal_length=24.0, focus_distance=400.0, horizontal_aperture=20.955, clipping_range=(0.1, 2)
                focal_length=24.0, focus_distance=400.0, horizontal_aperture=20.955, clipping_range=(0.0001, 2)
            ),
            offset=CameraCfg.OffsetCfg(
                # # 기본 세팅
                ...
                # # 정면에서 눈높이로
                # pos=(1.5, 0.0, 0.2), rot=(0.5, -0.5, -0.5, 0.5), convention="ros"
