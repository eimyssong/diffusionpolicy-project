...
def point_cloud(env, sensor_cfg):
    ...
    # colors = rgb.reshape(rgb.shape[0], -1, 3)
    colors = rgb.permute(0, 2, 1, 3).reshape(rgb.shape[0], -1, 3)

    if colors.dtype == torch.uint8:
        colors = colors.float() / 255.0

    pc = torch.cat([points_3d_world, colors], dim=-1)
